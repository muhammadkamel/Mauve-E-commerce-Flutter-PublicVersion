...
class HomeLayoutCubit extends Cubit<HomeLayoutStates> {
  ...
  void updateToken(BuildContext context) {
    FirebaseMessaging.instance.getToken().then((token) {
      TokenModel tokenModel = TokenModel(
          AuthCubit.get(context).userModel.phone, token!, false, false);
  ...
  void getCategoryList() {
    CategoryRepo categoryRepo =
        CategoryRepo(CategoryApi(categoryList: categoryList));
  ...
  void navigateToProductListByCategory(
      {required CategoriesModel categoryItem}) {
  ...
  void getProductsByCategoryMenuId({required CategoriesModel categoriesItem}) {
    productList = [];
    ProductsRepo productsRepo = ProductsRepo(ProductsApi(
        productList: productList, selectedCategoryItem: categoriesItem));
  ...
  void getInfo() {
    FirebaseFirestore.instance.collection(INFO_COLLECTION).get().then((value) {
  ...
  void submitUserMessage(
    ...
    MessagesModel model = MessagesModel(
        uid, name, phone, message, DateTime.now().millisecondsSinceEpoch);
    FirebaseFirestore.instance
